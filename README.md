...
1. **Backend (`roomiesmart-backend`)**: Node.js + Express + TypeScript + Supabase.
2. **Frontend Web (`roomiesmart-frontend`)**: React + Vite + TailwindCSS.
3. **Frontend Móvil (`roomiesmart-mobile`)**: React Native + Expo.
...
> Para que tu celular (o emulador) pueda comunicarse con el backend local, la app móvil necesita saber la **Dirección IP de tu computadora** en tu red Wi-Fi. 
> 
> 1. Abre una terminal y escribe `ipconfig` (Windows) o `ifconfig` (Mac/Linux) para averiguar tu IP IPv4 (ej. `192.168.1.15`).
> 2. Ve al archivo `roomiesmart-mobile/src/core/api.ts` y asegúrate de que la `BASE_URL` apunte a esa IP (ej. `http://192.168.1.15:3000`).

1. Abre una **tercera** terminal y navega a la carpeta móvil:
   ```bash
   cd roomiesmart-mobile
   ```
2. Instala las dependencias (se recomienda usar `npx expo install` para evitar conflictos nativos):
   ```bash
   npm install
   ```
3. Inicia el empaquetador de Expo limpiando la caché:
   ```bash
